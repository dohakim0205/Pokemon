...
class Play {
	...
	private ArrayList<Pokemon> monster;

	public Play() {
		this.monster = new ArrayList<Pokemon>();
		this.boss = new Boss();
		makePokemon();
		this.pokemon = this.monster.get(this.monster.size() - 1);
		this.SIZE = 10;
		this.ROAD = 0;
		this.PLAYER = 1;
		this.WALL = 2;
		this.hero = new Hero();
		if (this.pokemon.getPos() == this.boss.getPos()) {
			pokemon = new Pokemon();
		}
	...
	private Unit ismatchEnemy() {
		if (this.hero.getPos() == this.pokemon.getPos()) {
			System.out.printf("[!%s이(가) 나타났다!]\n", this.pokemon.getName());
			if (this.pokemon.getName().equals("귀여운 토게피")) {
				meetTogephy();
				return null;
			}

			if (this.pokemon.getName().equals("고라파덕")) {
				meetGorapaduck();
				return null;
			}
			return this.pokemon;
	...
	private void heroWin() {
		...
		else if (this.pokemon.getHp() <= 0) {
			makePokemon();
		}
		this.hero.setPos(0);
	}

	private void makePokemon() {
		String[] monsters = new String[] { "ggobuk", "pairi", "ggomadol", "mazayong", "togephi", "gorapaduck" };
		String className = "Pokemon_Game." + monsters[Unit.r.nextInt(monsters.length)];

		try {
			Class<?> clazz = Class.forName(className);
			Object object = clazz.getDeclaredConstructor().newInstance();
			if (object instanceof Pokemon) {
				Pokemon pokemone = (Pokemon) object;
				this.monster.add(pokemone);
				this.pokemon = this.monster.get(this.monster.size() - 1);
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (this.boss.getPos() == this.pokemon.getPos()) {
			makePokemon();
		}
	}
